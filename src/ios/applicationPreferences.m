...
- (void)clearSetting:(CDVInvokedUrlCommand*)command {
    NSString* jstring;
    NSString *settingsName = [command.arguments objectAtIndex:0];
    
    CDVPluginResult* result;
    @try {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:settingsName];
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        jstring = [result toSuccessCallbackString:command.callbackId];
    } @catch (NSException* e) {
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_NO_RESULT messageAsString:[e reason]];
        jstring = [result toErrorCallbackString:command.callbackId];
    } @finally {
